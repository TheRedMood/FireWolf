#!/usr/bin/env python
"""
ragnarok.py - A collection of commands for ragnarok online.
...
# This parses the text....... Need more 
def text(html): 
   html = r_tag.sub('', html)
...
def irowiki(term, last=False): 
   ...
   irouri = irouri.decode('utf-8').encode('utf-8')
...
def irowik(phenny, input): 
   ...
   try: result = irowiki(term)
   except IOError: 
	  args = (irouri % term)
	  error = "Can't connect to irowiki.org (%s)" % args
